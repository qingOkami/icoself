		...
		return this.each(function() {
			var _SELF = $(this),
				...
				fnGetIndexArr = function(index,len){  // 当前索引,长度
					...
					console.log(index)
					// $("#demo li").eq(index).css('overflow','visible')
					for(b=0;b<=len;b++){
				...
				object = function(length){
					...
					this.arrow = function(l,r,w,h,x,y){
						return "<em class=\"sPrev\" id=\"sPrev\" style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+195+"px;left:34%;\"></em><em class=\"sNext\" id=\"sNext\" style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+195+"px;left:62%;\"></em>";
					};
				...
				for(a=0;a<this.length;a++){
					...
					for(c=0;c<options.count;c++){
						...
						if(a===this.indexArr[c]){
							...
							if(a==CenIdxArr+1){
								t+=2;
								left = (options.width-width)+left+80;
                                width = options.width-50
                                top = options.imgtop
                                height =135
							};
                            if(a==CenIdxArr+2){
                                t+=2;
                                left = (options.width-width)+left-30;
                                width = options.width-50
                                top = options.imgtop
                                height =135
                            };
